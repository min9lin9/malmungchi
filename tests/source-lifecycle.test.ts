...
describe("source lifecycle", () => {
  it("rejects malformed source id export", async () => {
    ...
    const exportRes = await app.handle(
      new Request("http://localhost/malmungchi/sources/not-valid/export")
    );
  ...
  it("lists, inspects, and deletes an imported author source", async () => {
    ...
    const listRes = await app.handle(new Request("http://localhost/malmungchi/sources"));
    ...
    const detailRes = await app.handle(
      new Request("http://localhost/malmungchi/sources/author:demo-author")
    );
    ...
    const refreshRes = await app.handle(
      new Request("http://localhost/malmungchi/sources/author:demo-author/refresh", {
        method: "POST",
      })
    );
    ...
    const exportRes = await app.handle(
      new Request(
        "http://localhost/malmungchi/sources/author:demo-author/export?format=json&includeHistory=true"
      )
    );
    ...
    const historyRes = await app.handle(
      new Request("http://localhost/malmungchi/sources/author:demo-author/history?limit=2&offset=1")
    );
    ...
    const listAfterDelete = await app.handle(new Request("http://localhost/malmungchi/sources"));
    ...
    const historyAfterDeleteRes = await app.handle(
      new Request("http://localhost/malmungchi/sources/author:demo-author/history")
    );
